...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ── Mot de passe oublié ──
        ...
        if (empty($email)) {
            ...
            $success = 'Si cette adresse est connue, un lien de réinitialisation vous a été envoyé.';
        }
        $tab = 'forgot';
    }
}
...
<head>
  ...
  <title>
    <?= $tab === 'register' ? 'Inscription' : ($tab === 'forgot' ? 'Mot de passe oublié' : 'Connexion') ?>
    — Seek &amp; Stay
  </title>
  <meta name="description" content="Connexion et inscription à votre compte Seek &amp; Stay" />
  ...
  <style>
    ...
    @media (max-width: 500px) {
      .auth-card { padding: 30px 20px; border-radius: 10px; }
...
<body>
  ...
  <section class="auth-section">
    <div class="auth-card">
      ...
      <div class="auth-logo">
        <div class="auth-logo-icon">
          <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M9 21V12h6v9" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.5"/>
          </svg>
        </div>
        <h2>Seek &amp; Stay</h2>
        <p>
      ...
      <?php if (!empty($error)): ?>
        <div class="auth-message error"><?= $icon_alert ?> <?= htmlspecialchars($error) ?></div>
      <?php endif; ?>
      <?php if (!empty($success)): ?>
        <div class="auth-message success"><?= $icon_check ?> <?= htmlspecialchars($success) ?></div>
      <?php endif; ?>
      ...
      <form method="POST" novalidate>
        ...
        <div class="auth-form-group">
          <label for="telephone">
            Téléphone
            <span class="auth-optional">(facultatif)</span>
          </label>
  ...
  <footer>
    <div class="footer-top">
      ...
      <div class="footer-center">
        <p class="footer-text">
          Ce site a été conçu et développé par les élèves de l'ISEP du groupe
          G9A 2025/2026 en utilisant HTML, CSS, JavaScript et PHP - Tous
          droits réservés
        </p>
      </div>
    </div>
